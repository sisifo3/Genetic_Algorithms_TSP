function[fitness,anser,emigrate] = society_dolphin(g,celda_num,migration,immigrate,poblation_Ra,rnd)
...
if migration(1,1) == 1
    ...
    celda_num{rnd,1} = poblation_Ra{rnd,1};
    %celda_num{rnd,1} = immigrate;
    disp("migration dolphi")
    disp(rnd)
end
...
if migration(1,2) == 1
    ...
    celda_num{100,1} = poblation_Ra{1,1};
    disp('migration bw')
end    
...
for iter = 1:generations
    for rip = 1:mid_sol
        [pred1,pred2] = sel_pad_best_dolphin(celda_num);
        
        [desc1,desc2] = crossover_PMx_18_dolphin(pred1,pred2);
